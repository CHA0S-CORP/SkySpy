...
export function useSafetyEvents(
  ...
  const acknowledgeEventsBySeverity = useCallback(
    (severity) => {
      setAcknowledgedEvents((prev) => {
        const next = new Set(prev);
        safetyEvents.filter((e) => e.severity === severity).forEach((e) => next.add(e.id));
        return next;
      });
    },
    [safetyEvents]
  ...
  useEffect(() => {
    if (proximityConflicts.length === 0 && acknowledgedConflicts.size > 0) {
      ...
      const activeKeys = new Set(
        proximityConflicts.flatMap((c) => [`${c.hex1}-${c.hex2}`, `${c.hex2}-${c.hex1}`])
      );
  ...
  const isAircraftInConflict = useCallback(
    (hex) => {
      const upperHex = hex?.toUpperCase();
      return (
        activeConflicts.some(
          (e) => e.icao?.toUpperCase() === upperHex || e.icao_2?.toUpperCase() === upperHex
        ) ||
        proximityConflicts.some(
          (c) => c.hex1?.toUpperCase() === upperHex || c.hex2?.toUpperCase() === upperHex
        )
      );
    },
  ...
  const getAircraftConflict = useCallback(
    (hex) => {
      const upperHex = hex?.toUpperCase();
      return (
