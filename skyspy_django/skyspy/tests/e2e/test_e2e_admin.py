...
class TestUserManagementList:
    ...
    def test_list_users_includes_roles_and_permissions(self, superadmin_client, create_user, all_roles):
        ...
        # Find the test user
        test_user = next((u for u in result["users"] if u["username"] == "role_test_user"), None)
...
class TestUserManagementUpdate:
    ...
    def test_superadmin_can_update_user(self, superadmin_client, create_user, all_roles):
        ...
        response = superadmin_client.patch(f"/api/v1/admin/users/{user.id}/", data, format="json")
    ...
    def test_update_user_active_status(self, superadmin_client, create_user, all_roles):
        ...
        # Disable user
        response = superadmin_client.patch(f"/api/v1/admin/users/{user.id}/", {"is_active": False}, format="json")
        ...
        # Re-enable user
        response = superadmin_client.patch(f"/api/v1/admin/users/{user.id}/", {"is_active": True}, format="json")
    ...
    def test_admin_can_update_non_admin_users(self, admin_client, create_user, all_roles):
        ...
        response = admin_client.patch(
            f"/api/v1/admin/users/{user.id}/", {"display_name": "Admin Updated"}, format="json"
        )
    ...
    def test_cannot_change_username_to_existing(self, superadmin_client, create_user, all_roles):
        ...
        response = superadmin_client.patch(
            f"/api/v1/admin/users/{user.id}/", {"username": "existing_user"}, format="json"
        )
...
class TestRoleAssignment:
    ...
    def test_assign_role_to_user(self, superadmin_client, create_user, all_roles):
        ...
        response = superadmin_client.post(
            f"/api/v1/admin/users/{user.id}/assign_role/", {"role_id": all_roles["operator"].id}, format="json"
        )
    ...
    def test_assign_nonexistent_role_returns_404(self, superadmin_client, create_user):
        ...
        response = superadmin_client.post(
            f"/api/v1/admin/users/{user.id}/assign_role/", {"role_id": 99999}, format="json"
        )
    ...
    def test_assign_already_assigned_role(self, superadmin_client, create_user, all_roles):
        ...
        response = superadmin_client.post(
            f"/api/v1/admin/users/{user.id}/assign_role/", {"role_id": all_roles["viewer"].id}, format="json"
        )
    ...
    def test_admin_cannot_assign_superadmin_role(self, admin_client, create_user, all_roles):
        ...
        response = admin_client.post(
            f"/api/v1/admin/users/{user.id}/assign_role/", {"role_id": all_roles["superadmin"].id}, format="json"
        )
...
class TestRoleRemoval:
    ...
    def test_remove_role_from_user(self, superadmin_client, create_user, all_roles):
        ...
        response = superadmin_client.post(
            f"/api/v1/admin/users/{user.id}/remove_role/", {"role_id": all_roles["operator"].id}, format="json"
        )
    ...
    def test_remove_nonexistent_role_returns_404(self, superadmin_client, create_user):
        ...
        response = superadmin_client.post(
            f"/api/v1/admin/users/{user.id}/remove_role/", {"role_id": 99999}, format="json"
        )
...
class TestRoleManagementList:
    ...
    def test_roles_include_permissions(self, superadmin_client, all_roles):
        ...
        viewer_role = next((r for r in result["roles"] if r["name"] == "viewer"), None)
...
class TestRoleManagementDelete:
    ...
    def test_delete_role_with_assignments_cascades_or_fails(self, superadmin_client, create_user, db):
...
class TestUserRoleAssignmentsList:
    ...
    def test_filter_assignments_by_role(self, superadmin_client, create_user, all_roles):
        ...
        response = superadmin_client.get(f"/api/v1/admin/user-roles/?role_id={all_roles['viewer'].id}")
    ...
    def test_assignments_show_expiration_status(self, superadmin_client, create_user, all_roles):
        ...
        UserRole.objects.create(user=user, role=all_roles["viewer"], expires_at=timezone.now() - timedelta(days=1))

        # Create active assignment
        UserRole.objects.create(user=user, role=all_roles["operator"], expires_at=timezone.now() + timedelta(days=30))

        # Create permanent assignment
        UserRole.objects.create(user=user, role=all_roles["analyst"], expires_at=None)
        ...
        for assignment in result["assignments"]:
            ...
            if "is_expired" in assignment and assignment["role_name"] == "viewer":
...
class TestAPIKeyList:
    ...
    def test_superadmin_can_list_all_api_keys(self, superadmin_client, create_user, create_api_key, all_roles):
    ...
    def test_api_key_list_hides_full_key(self, superadmin_client, operator_user, create_api_key):
        ...
        # Find our key
        key_data = next((k for k in result["api_keys"] if k["name"] == "Hidden Key"), None)
    ...
    def test_filter_api_keys_by_user(self, superadmin_client, create_user, create_api_key, all_roles):
...
class TestAPIKeyCreate:
    ...
    def test_api_key_only_shown_on_creation(self, superadmin_client, create_user, all_roles):
...
class TestAPIKeyDelete:
    ...
    def test_admin_can_revoke_non_admin_keys(self, admin_client, create_user, create_api_key, all_roles):
...
class TestFeatureAccessList:
    ...
    def test_list_feature_access_configurations(self, superadmin_client, db):
        ...
        aircraft = next((f for f in result["features"] if f["feature"] == "aircraft"), None)
...
class TestFeatureAccessUpdate:
    ...
    def test_update_read_access_to_public(self, superadmin_client, db):
        ...
        response = superadmin_client.patch(
            "/api/v1/admin/feature-access/alerts/", {"read_access": "public"}, format="json"
        )
    ...
    def test_update_write_access_to_authenticated(self, superadmin_client, db):
        ...
        response = superadmin_client.patch(
            "/api/v1/admin/feature-access/alerts/", {"write_access": "authenticated"}, format="json"
        )
    ...
    def test_disable_feature(self, superadmin_client, db):
        ...
        response = superadmin_client.patch("/api/v1/admin/feature-access/acars/", {"is_enabled": False}, format="json")
    ...
    def test_enable_feature(self, superadmin_client, db):
        ...
        response = superadmin_client.patch("/api/v1/admin/feature-access/audio/", {"is_enabled": True}, format="json")
    ...
    def test_update_nonexistent_feature_returns_404(self, superadmin_client):
        ...
        response = superadmin_client.patch(
            "/api/v1/admin/feature-access/nonexistent/", {"read_access": "public"}, format="json"
        )
    ...
    def test_invalid_access_level_rejected(self, superadmin_client, db):
        ...
        response = superadmin_client.patch(
            "/api/v1/admin/feature-access/safety/", {"read_access": "invalid_level"}, format="json"
        )
...
class TestOIDCClaimMappingsList:
    ...
    def test_list_oidc_claim_mappings(self, superadmin_client, all_roles):
        ...
        mapping = next((m for m in result["mappings"] if m["name"] == "Admins Group"), None)
...
class TestOIDCClaimMappingUpdate:
    ...
    def test_update_mapping_claim_value(self, superadmin_client, all_roles):
        ...
        response = superadmin_client.patch(
            f"/api/v1/admin/oidc-mappings/{mapping.id}/", {"claim_value": "new-value"}, format="json"
        )
    ...
    def test_update_mapping_match_type(self, superadmin_client, all_roles):
        ...
        response = superadmin_client.patch(
            f"/api/v1/admin/oidc-mappings/{mapping.id}/", {"match_type": "contains"}, format="json"
        )
    ...
    def test_update_mapping_active_status(self, superadmin_client, all_roles):
        ...
        # Deactivate
        response = superadmin_client.patch(
            f"/api/v1/admin/oidc-mappings/{mapping.id}/", {"is_active": False}, format="json"
        )
...
class TestAdminPermissionChecks:
    ...
    def test_viewer_cannot_access_any_admin_endpoints(self, viewer_client):
        ...
        for endpoint in endpoints:
            ...
            assert response.status_code == status.HTTP_403_FORBIDDEN, f"Viewer should not access {endpoint}"
    ...
    def test_operator_cannot_access_admin_endpoints(self, operator_client):
        ...
        for endpoint in endpoints:
            ...
            assert response.status_code == status.HTTP_403_FORBIDDEN, f"Operator should not access {endpoint}"
    ...
    def test_analyst_cannot_access_admin_endpoints(self, analyst_client):
        ...
        for endpoint in endpoints:
            ...
            assert response.status_code == status.HTTP_403_FORBIDDEN, f"Analyst should not access {endpoint}"
    ...
    def test_unauthenticated_gets_401_on_admin_endpoints(self, api_client):
        ...
        for endpoint in endpoints:
            ...
            assert response.status_code == status.HTTP_401_UNAUTHORIZED, f"Unauthenticated should not access {endpoint}"
...
class TestAdminAuditLogging:
    ...
    def test_user_creation_is_logged(self, superadmin_client, all_roles):
        ...
        with patch("skyspy.api.admin.logger"):
            response = superadmin_client.post("/api/v1/admin/users/", data, format="json")
    ...
    def test_role_assignment_is_logged(self, superadmin_client, superadmin_user, create_user, all_roles):
    ...
    def test_user_deletion_is_logged(self, superadmin_client, create_user, all_roles):
    ...
    def test_api_key_creation_is_logged(self, superadmin_client, create_user, all_roles):
...
class TestAdminIntegrationWorkflows:
    ...
    def test_complete_user_lifecycle(self, superadmin_client, all_roles):
        ...
        assign_response = superadmin_client.post(
            f"/api/v1/admin/users/{user_id}/assign_role/", {"role_id": all_roles["operator"].id}, format="json"
        )
        assert assign_response.status_code == status.HTTP_200_OK

        # 3. Update user
        update_response = superadmin_client.patch(
            f"/api/v1/admin/users/{user_id}/", {"display_name": "Lifecycle Test User"}, format="json"
        )
        assert update_response.status_code == status.HTTP_200_OK

        # 4. Disable user
        disable_response = superadmin_client.patch(
            f"/api/v1/admin/users/{user_id}/", {"is_active": False}, format="json"
        )
    ...
    def test_create_custom_role_and_assign_to_users(self, superadmin_client, create_user):
        ...
        assign1 = superadmin_client.post(
            f"/api/v1/admin/users/{user1.id}/assign_role/", {"role_id": role_id}, format="json"
        )
        assert assign1.status_code == status.HTTP_200_OK

        assign2 = superadmin_client.post(
            f"/api/v1/admin/users/{user2.id}/assign_role/", {"role_id": role_id}, format="json"
        )
    ...
    def test_configure_feature_access_for_public_deployment(self, superadmin_client, db):
        ...
        superadmin_client.patch("/api/v1/admin/feature-access/aircraft/", {"read_access": "public"}, format="json")
        superadmin_client.patch("/api/v1/admin/feature-access/safety/", {"read_access": "public"}, format="json")

        # 2. Disable ACARS for this deployment
        superadmin_client.patch("/api/v1/admin/feature-access/acars/", {"is_enabled": False}, format="json")
