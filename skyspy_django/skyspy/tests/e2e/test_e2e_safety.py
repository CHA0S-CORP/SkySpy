...
class TestSafetyEventListing:
    ...
    def test_filter_by_event_type_tcas_ta(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(
            event_type="tcas_ta", severity="warning", icao_hex="TCASTA1", message="TCAS TA detected"
        )
        SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="TCASRA1", message="TCAS RA detected"
        )
    ...
    def test_filter_by_event_type_7500_hijack(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(
            event_type="7500", severity="critical", icao_hex="HJK001", message="Squawk 7500 - Hijack"
        )
    ...
    def test_filter_by_event_type_7600_radio_failure(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(
            event_type="7600", severity="warning", icao_hex="RF001", message="Squawk 7600 - Radio Failure"
        )
    ...
    def test_filter_by_event_type_7700_emergency(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(
            event_type="7700", severity="critical", icao_hex="EMG001", message="Squawk 7700 - Emergency"
        )
    ...
    def test_filter_by_event_type_extreme_vs(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(
            event_type="extreme_vs", severity="warning", icao_hex="VS001", message="Extreme vertical speed detected"
        )
    ...
    def test_filter_by_severity_info(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(event_type="tcas_ta", severity="info", icao_hex="INFO01", message="Info level event")
    ...
    def test_filter_by_acknowledged_true(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(event_type="extreme_vs", severity="warning", icao_hex="UNACK01", acknowledged=False)
    ...
    def test_filter_by_acknowledged_false(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(event_type="extreme_vs", severity="warning", icao_hex="UNACK02", acknowledged=False)
    ...
    def test_filter_by_icao_hex(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="TARGETICAO", callsign="TGT123")
        SafetyEvent.objects.create(event_type="extreme_vs", severity="warning", icao_hex="OTHERICAO", callsign="OTH456")
    ...
    def test_list_ordered_by_timestamp_descending(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="FIRST01")
        SafetyEvent.objects.create(event_type="extreme_vs", severity="warning", icao_hex="SECOND1")
    ...
    def test_combine_multiple_filters(self, viewer_client, db):
        ...
        SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="MULTI01", acknowledged=False)
        SafetyEvent.objects.create(event_type="tcas_ra", severity="warning", icao_hex="MULTI02", acknowledged=False)
        SafetyEvent.objects.create(event_type="extreme_vs", severity="critical", icao_hex="MULTI03", acknowledged=False)

        response = viewer_client.get("/api/v1/safety/events/?event_type=tcas_ra&severity=critical&acknowledged=false")
        data = response.json()
...
class TestSafetyEventDetails:
    ...
    def test_retrieve_event_data(self, viewer_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="DETAIL2", callsign="DTL456", message="TCAS RA: Climb"
        )
    ...
    def test_retrieve_event_with_aircraft_snapshot(self, viewer_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="SNAP001", callsign="SNP123", aircraft_snapshot=snapshot
        )
...
class TestEventAcknowledgment:
    ...
    def test_acknowledge_event(self, operator_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="ACK001", acknowledged=False
        )
    ...
    def test_acknowledge_sets_timestamp(self, operator_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="ACK002", acknowledged=False
        )
    ...
    def test_acknowledge_returns_updated_event(self, operator_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="ACK003", acknowledged=False
        )
    ...
    def test_viewer_can_read_but_not_acknowledge(self, viewer_client, feature_access_permission_based, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="ACK005", acknowledged=False
        )
...
class TestEventDeletion:
    ...
    def test_delete_event(self, admin_client, db):
        ...
        event = SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="DEL001")
    ...
    def test_delete_removes_from_database(self, admin_client, db):
        ...
        event = SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="DEL002")
        event_id = event.id
...
class TestRealTimeDetectionWorkflow:
    ...
    def test_tcas_ra_creates_critical_event(self, db):
        ...
        [e for e in events if e.get("event_type") in ["tcas_ra", "vs_reversal"]]
    ...
    def test_proximity_conflict_detects_close_aircraft(self, db):
        ...
        [e for e in events if e.get("event_type") == "proximity_conflict"]
...
class TestPermissionChecks:
    ...
    def test_viewer_can_read_single_event(self, viewer_client, db):
        ...
        event = SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="VIEWR1")
        response = viewer_client.get(f"/api/v1/safety/events/{event.id}/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_operator_can_acknowledge_events(self, operator_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="OPRAC1", acknowledged=False
        )
        response = operator_client.post(f"/api/v1/safety/events/{event.id}/acknowledge/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_admin_can_delete_events(self, admin_client, db):
        ...
        event = SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="ADMND1")
        response = admin_client.delete(f"/api/v1/safety/events/{event.id}/")
        assert response.status_code == status.HTTP_204_NO_CONTENT

    def test_unauthenticated_gets_403_on_protected_endpoint(self, api_client, feature_access_permission_based, db):
        """Test that unauthenticated user gets 403 on protected endpoints."""
        SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="NOAUTH")

        # In permission-based mode, unauthenticated users should be denied
        api_client.get("/api/v1/safety/events/")
    ...
    def test_no_role_user_has_limited_access(self, no_role_client, feature_access_permission_based, db):
        ...
        SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="NOROL1")

        # User without roles should have limited access in permission mode
        no_role_client.get("/api/v1/safety/events/")
    ...
    def test_superadmin_has_full_access(self, superadmin_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="SUPER1", acknowledged=False
        )
...
class TestSafetyEventsIntegration:
    ...
    def test_complete_acknowledge_workflow(self, operator_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="WKFLW1", acknowledged=False
        )
    ...
    def test_all_endpoints_return_json(self, viewer_client, db):
        ...
        event = SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="JSON01")
        ...
        for endpoint in endpoints:
            ...
            assert "application/json" in response["Content-Type"], f"Endpoint {endpoint} should return JSON"

    def test_http_methods_restricted(self, viewer_client, db):
        """Test that only allowed HTTP methods work."""
        event = SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="METH01")

        # PUT should not be allowed on events
        response = viewer_client.put(f"/api/v1/safety/events/{event.id}/", {"event_type": "extreme_vs"}, format="json")
        assert response.status_code == status.HTTP_405_METHOD_NOT_ALLOWED

        # PATCH should not be allowed
        response = viewer_client.patch(f"/api/v1/safety/events/{event.id}/", {"severity": "info"}, format="json")
        assert response.status_code == status.HTTP_405_METHOD_NOT_ALLOWED
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_special_characters_in_callsign(self, viewer_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="SPEC01", callsign="N12-34A"
        )
    ...
    def test_concurrent_acknowledge_requests(self, operator_client, db):
        ...
        event = SafetyEvent.objects.create(
            event_type="tcas_ra", severity="critical", icao_hex="CONC01", acknowledged=False
        )
...
class TestAPIKeyAuthentication:
    ...
    def test_api_key_can_read_events(self, api_key_client, db):
        ...
        SafetyEvent.objects.create(event_type="tcas_ra", severity="critical", icao_hex="APIKEY")
