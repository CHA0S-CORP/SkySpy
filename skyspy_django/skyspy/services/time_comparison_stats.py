...
from django.db.models.functions import ExtractHour, ExtractWeekDay, TruncDay, TruncMonth, TruncWeek
...
def calculate_week_over_week_comparison() -> dict:
    ...
    def get_week_stats(start: datetime, end: datetime) -> dict:
        ...
        sightings = AircraftSighting.objects.filter(timestamp__gte=start, timestamp__lt=effective_end)

        sessions = AircraftSession.objects.filter(first_seen__gte=start, first_seen__lt=effective_end)
    ...
    def calc_change(current: int, previous: int) -> dict:
        ...
        pct = (100.0 if current > 0 else 0.0) if previous == 0 else (current - previous) / previous * 100
        return {
...
def calculate_day_night_ratio(days: int = 30) -> dict:
    ...
    day_aircraft = (
        sightings.annotate(hour=ExtractHour("timestamp"))
        .filter(hour__gte=DAY_START_HOUR, hour__lt=DAY_END_HOUR)
        .values("icao_hex")
        .distinct()
        .count()
    )
    ...
    night_aircraft = (
        sightings.annotate(hour=ExtractHour("timestamp"))
        .exclude(hour__gte=DAY_START_HOUR, hour__lt=DAY_END_HOUR)
        .values("icao_hex")
        .distinct()
        .count()
    )
    ...
    return {
        ...
        "ratio": {
            "day_to_night": round(day_stats["positions"] / night_stats["positions"], 2)
            if night_stats["positions"] > 0
            else None,
...
def calculate_weekend_weekday_patterns(weeks: int = 4) -> dict:
    ...
    daily_data = (
        sightings.annotate(weekday=ExtractWeekDay("timestamp"), day=TruncDay("timestamp"))
        .values("day", "weekday")
        .annotate(
            position_count=Count("id"),
            unique_aircraft=Count("icao_hex", distinct=True),
...
def calculate_daily_totals(days: int = 30) -> dict:
    ...
    session_data = (
        AircraftSession.objects.filter(first_seen__gte=cutoff)
        .annotate(day=TruncDay("first_seen"))
        .values("day")
        .annotate(
            total_sessions=Count("id"),
            military_sessions=Count("id", filter=Q(is_military=True)),
...
def calculate_weekly_totals(weeks: int = 12) -> dict:
    ...
    session_data = (
        AircraftSession.objects.filter(first_seen__gte=cutoff)
        .annotate(week=TruncWeek("first_seen"))
        .values("week")
        .annotate(
            total_sessions=Count("id"),
            military_sessions=Count("id", filter=Q(is_military=True)),
...
def get_week_comparison() -> dict | None:
...
def get_seasonal_trends() -> dict | None:
...
def get_day_night_ratio() -> dict | None:
...
def get_weekend_weekday_patterns() -> dict | None:
...
def get_daily_totals(days: int = 30) -> dict | None:
...
def get_weekly_totals(weeks: int = 12) -> dict | None:
...
def get_monthly_totals(months: int = 12) -> dict | None:
