...
class NotificationRouter:
    ...
    def get_channels_for_notification(
        ...
        if user and user.is_authenticated:
            preferences = UserNotificationPreference.objects.filter(
                user=user, enabled=True, channel__enabled=True
        ...
        # 3. Global channels
        if include_global:
            global_channels = NotificationChannel.objects.filter(is_global=True, enabled=True)
    ...
    def get_channels_for_priority(self, priority: str, event_type: str = "alert") -> list[RoutedNotification]:
        """
        ...
        channels = NotificationChannel.objects.filter(is_global=True, enabled=True)
    ...
    def get_channels_for_users(
        self, user_ids: list[int], priority: str, event_type: str
        ...
        preferences = UserNotificationPreference.objects.filter(
            user_id__in=user_ids, enabled=True, channel__enabled=True
    ...
    def get_subscribers_for_rule(
        self, rule_id: int, priority: str, event_type: str = "alert"
    ) -> list[RoutedNotification]:
        """
        ...
        try:
            subscriptions = AlertSubscription.objects.filter(rule_id=rule_id, notify_on_trigger=True).select_related(
                "user"
            )
            ...
            for _user_id, channels in user_channels.items():
                for channel in channels:
    ...
    def should_notify(self, priority: str, min_priority: str) -> bool:
        """
