...
from skyspy.models import NotificationChannel, NotificationConfig, NotificationLog, SkyspyUser
...
class NotificationChannelViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response({"channels": serializer.data, "count": queryset.count()})
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(NotificationChannelSerializer(channel).data, status=status.HTTP_201_CREATED)
    ...
    @extend_schema(summary="Delete notification channel", responses={204: None})
    ...
    def test(self, request, pk=None):
        ...
        if not channel.enabled:
            return Response({"success": False, "message": "Channel is disabled", "servers_notified": 0})
        ...
        try:
            ...
            result = apobj.notify(title=title, body=message, notify_type=apprise.NotifyType.INFO)
    ...
    @extend_schema(summary="Get user's own channels", responses={200: NotificationChannelSerializer(many=True)})
    ...
    @extend_schema(
        summary="List available channel types", description="Get information about supported notification channel types"
    )
    @action(detail=False, methods=["get"])
    def types(self, request):
        """List available notification channel types with configuration info."""
        return Response({"types": CHANNEL_TYPES, "count": len(CHANNEL_TYPES)})
...
class NotificationViewSet(viewsets.ViewSet):
    ...
    @extend_schema(summary="Get notification config", responses={200: NotificationConfigSerializer})
    @action(detail=False, methods=["get"])
    ...
    def update_config(self, request):
        ...
        config = NotificationConfig.get_config()
        serializer = NotificationConfigUpdateSerializer(config, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(NotificationConfigSerializer(config).data)
    ...
    def test(self, request):
        ...
        if not config.apprise_urls:
            return Response({"success": False, "message": "No notification URLs configured", "servers_notified": 0})

        if not config.enabled:
            return Response({"success": False, "message": "Notifications are disabled", "servers_notified": 0})
    ...
    def services(self, request):
        ...
        services = [{"name": ct["name"], "schema": ct["schema"]} for ct in CHANNEL_TYPES]
    ...
    def history(self, request):
        ...
        logs = queryset[:limit]
        return Response({"history": NotificationLogSerializer(logs, many=True).data, "count": logs.count()})

    @extend_schema(summary="Get notification statistics", responses={200: dict})
    @action(detail=False, methods=["get"])
    def stats(self, request):
        ...
        stats = NotificationLog.objects.filter(timestamp__gte=cutoff).aggregate(
            total=Count("id"),
        ...
        # By type
        by_type = list(
            NotificationLog.objects.filter(timestamp__gte=cutoff)
        ...
        # By channel
        by_channel = list(
            NotificationLog.objects.filter(timestamp__gte=cutoff, channel__isnull=False)
            .values("channel__name", "channel_id")
            .annotate(count=Count("id"))
            .order_by("-count")[:10]
        )
