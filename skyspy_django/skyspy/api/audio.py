...
class AudioViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(summary="Queue transcription", description="Queue an audio transmission for transcription")
    ...
    @extend_schema(summary="Get audio statistics", responses={200: AudioStatsSerializer})
    ...
    def match_airframes(self, request, pk=None):
        ...
        if not transmission.transcript:
            return Response({"error": "No transcript available"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    @extend_schema(summary="Serve audio file", description="Serve audio file from local storage (for non-S3 setups)")
    ...
    @extend_schema(summary="Get audio URL", description="Get accessible URL for an audio transmission")
    ...
    def matched_calls(self, request):
        ...
        if not callsign and not operator_icao and not registration:
            return Response(
                {"error": "Must provide callsign, operator_icao, or registration"}, status=status.HTTP_400_BAD_REQUEST
            )
