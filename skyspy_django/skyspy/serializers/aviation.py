...
from skyspy.models import AirspaceAdvisory, AirspaceBoundary, CachedAirport, CachedGeoJSON, CachedNavaid, CachedPirep
...
class CachedPirepSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "report_type",
            "lat",
            "lon",  # Frontend-compatible field names
            "latitude",
            "longitude",  # Standard field names
            "location",
            "observation_time",
            "flight_level",
            "altitude_ft",
...
class AviationDataSerializer(serializers.Serializer):
    ...
    data = serializers.ListField(child=serializers.DictField(), help_text="Aviation data items")
    count = serializers.IntegerField(help_text="Number of items")
    source = serializers.CharField(default="aviationweather.gov", help_text="Data source")
    cached = serializers.BooleanField(default=False, help_text="Whether data is from cache")
    cache_age_seconds = serializers.IntegerField(required=False, allow_null=True, help_text="Cache age if cached")
