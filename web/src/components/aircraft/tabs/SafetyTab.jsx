...
export function SafetyTab({
  ...
  const updateSafetyReplayMarkers = useCallback(
    (eventId, position) => {
      ...
      if (data.track1?.length > 0) {
        ...
        if (pos) {
          ...
          safetyMarkersRef.current[`${eventId}_1`] = L.marker([pos.lat, pos.lon], { icon }).addTo(
            map
          );
      ...
      if (data.track2?.length > 0) {
        ...
        if (pos) {
          ...
          safetyMarkersRef.current[`${eventId}_2`] = L.marker([pos.lat, pos.lon], { icon }).addTo(
            map
          );
  ...
  const toggleSafetyMap = useCallback(
    async (eventId, event) => {
      ...
      if (isExpanding && !safetyTrackData[eventId]) {
        ...
        try {
          const fetchTracks = async (icao) => {
            ...
            if (wsRequest && wsConnected) {
              ...
              const res = await fetch(
                `${baseUrl}/api/v1/sightings?icao_hex=${icao}&hours=1&limit=500`
              );
          ...
          setSafetyReplayState((prev) => ({
            ...prev,
            [eventId]: { position: 50, isPlaying: false, speed: 1 },
          }));
        } catch (err) {
  ...
  const initializeSafetyMap = useCallback(
    (containerEl, eventId) => {
      ...
      if (event.lat && event.lon) {
        L.circleMarker([event.lat, event.lon], {
          ...
          .addTo(map)
          .bindPopup(
            `<b>Safety Event</b><br>${event.message || formatEventType(event.event_type)}`
          );
      }
  ...
  const handleSafetyReplayChange = useCallback(
    (eventId, newPosition) => {
      setSafetyReplayState((prev) => ({
        ...prev,
  ...
  const toggleSafetyPlay = useCallback(
    (eventId) => {
      setSafetyReplayState((prev) => {
        ...
        if (!current.isPlaying) {
          ...
          const animate = (currentTime) => {
            ...
            pos += increment;
            if (pos >= 100) {
              setSafetyReplayState((p) => ({
                ...p,
                [eventId]: { ...p[eventId], position: 100, isPlaying: false },
              }));
  ...
  const skipSafetyToStart = useCallback(
    (eventId) => {
      if (safetyAnimationRef.current[eventId])
        cancelAnimationFrame(safetyAnimationRef.current[eventId]);
      ...
      setSafetyReplayState((prev) => ({
        ...prev,
        [eventId]: { position: 0, isPlaying: false, speed: current.speed },
      }));
  ...
  const skipSafetyToEnd = useCallback(
    (eventId) => {
      if (safetyAnimationRef.current[eventId])
        cancelAnimationFrame(safetyAnimationRef.current[eventId]);
      const current = safetyReplayState[eventId] || { speed: 1 };
      setSafetyReplayState((prev) => ({
        ...prev,
        [eventId]: { position: 100, isPlaying: false, speed: current.speed },
      }));
      updateSafetyReplayMarkers(eventId, 100);
    },
    [safetyReplayState, setSafetyReplayState, updateSafetyReplayMarkers]
  ...
  const jumpToSafetyEvent = useCallback(
    (eventId) => {
      if (safetyAnimationRef.current[eventId])
        cancelAnimationFrame(safetyAnimationRef.current[eventId]);
      const current = safetyReplayState[eventId] || { speed: 1 };
      setSafetyReplayState((prev) => ({
        ...prev,
        [eventId]: { position: 50, isPlaying: false, speed: current.speed },
      }));
  ...
  const handleSafetySpeedChange = useCallback(
    (eventId, newSpeed) => {
      setSafetyReplayState((prev) => ({
        ...prev,
  ...
  const getSafetyReplayTimestamp = useCallback(
    (eventId) => {
      const data = safetyTrackData[eventId];
      ...
      <div className="snapshot-section">
        ...
        <div className="snapshot-grid">
          ...
          {snapshot.track !== undefined && snapshot.track !== null && (
            <div className="snapshot-item">
              <span>Track</span>
              <span>{snapshot.track?.toFixed(0)}°</span>
            </div>
          )}
          {snapshot.baro_rate && (
            <div className="snapshot-item">
      ...
      {safetyEvents.length === 0 ? (
        ...
        <div className="safety-events-list">
          <p className="safety-count" aria-live="polite">
            {safetyEvents.length} safety event{safetyEvents.length !== 1 ? 's' : ''} in the last{' '}
            {safetyHours} hour{safetyHours !== 1 ? 's' : ''}
          </p>
          {safetyEvents.map((event, i) => {
            ...
            return (
              <article
                key={eventKey}
                className={`safety-event-item ${getSeverityClass(event.severity)}`}
              >
                ...
                {event.details && (
                  <div className="safety-event-details">
                    ...
                    {event.details.vertical_rate && (
                      <span>
                        VS: {event.details.vertical_rate > 0 ? '+' : ''}
                        {event.details.vertical_rate}fpm
                      </span>
                    )}
                    ...
                    {event.aircraft_snapshot &&
                      renderSnapshot(
                        ...
                        event.aircraft_snapshot_2
                          ? event.aircraft_snapshot.flight || event.icao
                          : null
                      )}
                    {event.aircraft_snapshot_2 &&
                      renderSnapshot(
                  ...
                  <div className="safety-map-container">
                    <div
                      ...
                      ref={(el) => {
                        if (
                          el &&
                          expandedSafetyMaps[eventKey] &&
                          !safetyMapRefs.current[eventKey]
                        ) {
