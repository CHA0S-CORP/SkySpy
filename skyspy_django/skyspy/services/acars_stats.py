...
def calculate_acars_message_stats(hours: int = 24) -> dict:
    ...
    # Count by source
    by_source = dict(base_qs.values_list("source").annotate(count=Count("id")))

    # Count by label
    label_counts_raw = list(base_qs.values("label").annotate(count=Count("id")).order_by("-count"))
    ...
    for cat_key, count in sorted(category_counts.items(), key=lambda x: x[1], reverse=True):
        cat_info = MESSAGE_CATEGORIES.get(
            cat_key,
            {
    ...
    top_frequencies = list(
        base_qs.filter(frequency__isnull=False).values("frequency").annotate(count=Count("id")).order_by("-count")[:10]
    )
    ...
    with_decoded = base_qs.filter(Q(decoded__isnull=False) | Q(text__isnull=False)).exclude(text="").count()
...
def calculate_acars_airline_stats(hours: int = 24, limit: int = 20) -> dict:
    ...
    callsign_counts = list(
        base_qs.values("callsign").annotate(count=Count("id")).order_by("-count")[:500]  # Get more for aggregation
    )

    # Aggregate by airline
    airline_stats = defaultdict(
        lambda: {
            "count": 0,
            "flights": set(),
            "icao": None,
...
def calculate_acars_trends(hours: int = 24, interval: str = "hour") -> dict:
    ...
    trends_raw = (
        base_qs.annotate(interval_start=trunc_func("timestamp"))
    ...
    hourly_distribution = (
        base_qs.annotate(hour=ExtractHour("timestamp")).values("hour").annotate(count=Count("id")).order_by("hour")
    )
...
def calculate_acars_category_trends(hours: int = 24) -> dict:
    ...
    hourly_labels = list(
        base_qs.annotate(hour=ExtractHour("timestamp"))
        .values("hour", "label")
...
def calculate_free_text_analysis(hours: int = 24, limit: int = 20) -> dict:
    ...
    messages = (
        AcarsMessage.objects.filter(
            timestamp__gte=cutoff,
            text__isnull=False,
        )
        .exclude(text="")
        .values_list("text", "label")[:5000]
    )
    ...
    return {
        "top_airports_mentioned": [
            {"airport": apt, "count": count} for apt, count in airport_mentions.most_common(limit)
        ],
...
def get_cached_acars_stats() -> dict | None:
...
def get_cached_acars_trends() -> dict | None:
...
def get_cached_acars_airlines() -> dict | None:
