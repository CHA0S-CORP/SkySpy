...
def update_antenna_analytics():
    ...
    sightings = AircraftSighting.objects.filter(timestamp__gte=cutoff, latitude__isnull=False, longitude__isnull=False)
    ...
    for s in sightings.values("latitude", "longitude", "distance_nm", "rssi", "icao_hex"):
        ...
        math.radians(lat - feeder_lat)
        dlon = math.radians(lon - feeder_lon)

        x = math.sin(dlon) * math.cos(math.radians(lat))
        y = math.cos(math.radians(feeder_lat)) * math.sin(math.radians(lat)) - math.sin(
            math.radians(feeder_lat)
        ) * math.cos(math.radians(lat)) * math.cos(dlon)
    ...
    max_range_by_direction = {
        sector: data["max_range"] for sector, data in range_by_direction.items() if data["max_range"] is not None
    }
    ...
    if stats["max_distance"]:
        logger.debug(
            f"Updated antenna analytics: max range {stats['max_distance']:.1f}nm, coverage {coverage_percentage}%"
        )
...
def cleanup_antenna_analytics_snapshots(retention_days: int = 7):
    ...
    try:
        ...
        deleted_scheduled, _ = AntennaAnalyticsSnapshot.objects.filter(
            timestamp__lt=cutoff, snapshot_type="scheduled"
        ).delete()
...
def aggregate_hourly_antenna_analytics():
    ...
    try:
        ...
        existing = AntennaAnalyticsSnapshot.objects.filter(timestamp=hour_end, snapshot_type="hourly").exists()
        ...
        snapshots = AntennaAnalyticsSnapshot.objects.filter(
            timestamp__gte=hour_start, timestamp__lt=hour_end, snapshot_type="scheduled"
        )
...
def update_favorite_tracking():
    ...
    try:
        ...
        recent_sessions = AircraftSession.objects.filter(last_seen__gte=cutoff).values(
            "icao_hex", "total_positions", "first_seen", "last_seen"
        )

        session_map = {s["icao_hex"]: s for s in recent_sessions}

        # Update favorites for active aircraft
        favorites_to_update = AircraftFavorite.objects.filter(icao_hex__in=session_map.keys())
