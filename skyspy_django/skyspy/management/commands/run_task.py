...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("task_name", nargs="?", help="Full task name (e.g., skyspy.tasks.aircraft.poll_aircraft)")
        parser.add_argument("--list", action="store_true", help="List all available tasks")
        parser.add_argument(
            "--async", action="store_true", dest="run_async", help="Queue the task instead of running synchronously"
        )
        parser.add_argument(
            "--args", type=str, default="[]", help='JSON array of positional arguments (e.g., \'["arg1", "arg2"]\')'
        )
        parser.add_argument(
            "--kwargs", type=str, default="{}", help='JSON object of keyword arguments (e.g., \'{"key": "value"}\')'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            matches = [t for t in app.tasks if task_name in t and not t.startswith("celery.")]
            if matches:
