...
from skyspy.models import AlertAggregate, AlertHistory, AlertRule, AlertSubscription, NotificationChannel
...
class SuppressionWindowSerializer(serializers.Serializer):
    ...
    day = serializers.CharField(
        required=False, allow_blank=True, help_text="Day of week (monday, tuesday, etc.) or empty for all days"
    )
...
class AlertRuleCreateSerializer(serializers.Serializer):
    ...
    type = serializers.CharField(
        source="rule_type", required=False, allow_null=True, max_length=30, help_text="Simple rule type"
    )
    operator = serializers.ChoiceField(
        choices=[(op, op) for op in VALID_OPERATORS], default="eq", help_text="Comparison operator"
    )
    value = serializers.CharField(max_length=100, required=False, allow_null=True, help_text="Comparison value")
    conditions = ComplexConditionsSerializer(required=False, allow_null=True, help_text="Complex conditions")
    description = serializers.CharField(max_length=200, required=False, default="", help_text="Rule description")
    enabled = serializers.BooleanField(default=True, help_text="Rule enabled status")
    priority = serializers.ChoiceField(
        choices=["info", "warning", "critical"], default="info", help_text="Alert priority"
    )
    starts_at = serializers.DateTimeField(required=False, allow_null=True, help_text="Rule activation time")
    expires_at = serializers.DateTimeField(required=False, allow_null=True, help_text="Rule expiration time")
    api_url = serializers.CharField(
        max_length=500, required=False, allow_null=True, help_text="External API to fetch when triggered"
    )
    ...
    visibility = serializers.ChoiceField(
        choices=["private", "shared", "public"], default="private", help_text="Who can see this rule"
    )
    suppression_windows = SuppressionWindowSerializer(
        many=True, required=False, default=list, help_text="Time windows when rule should not trigger"
    )
...
class AlertRuleUpdateSerializer(serializers.Serializer):
    ...
    name = serializers.CharField(max_length=100, required=False, help_text="Rule name")
    operator = serializers.CharField(max_length=10, required=False, help_text="Comparison operator")
    value = serializers.CharField(max_length=100, required=False, allow_null=True, help_text="Comparison value")
    conditions = serializers.JSONField(required=False, allow_null=True, help_text="Complex conditions")
    description = serializers.CharField(max_length=200, required=False, help_text="Rule description")
    enabled = serializers.BooleanField(required=False, help_text="Enabled status")
    priority = serializers.ChoiceField(
        choices=["info", "warning", "critical"], required=False, help_text="Priority level"
    )
    starts_at = serializers.DateTimeField(required=False, allow_null=True, help_text="Activation time")
    expires_at = serializers.DateTimeField(required=False, allow_null=True, help_text="Expiration time")
    api_url = serializers.CharField(max_length=500, required=False, allow_null=True, help_text="External API URL")
    cooldown_minutes = serializers.IntegerField(
        required=False, min_value=0, max_value=1440, help_text="Cooldown in minutes"
    )
    visibility = serializers.ChoiceField(
        choices=["private", "shared", "public"], required=False, help_text="Rule visibility"
    )
    suppression_windows = serializers.JSONField(required=False, help_text="Suppression time windows")
    notification_channel_ids = serializers.ListField(
        child=serializers.IntegerField(), required=False, help_text="IDs of notification channels to use for this rule"
    )
    use_global_notifications = serializers.BooleanField(
...
class AlertSubscriptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "rule_id", "rule_name", "rule_priority", "notify_on_trigger", "created_at"]
...
class AlertRuleTestSerializer(serializers.Serializer):
    ...
    aircraft = serializers.ListField(
        child=serializers.DictField(), required=False, help_text="List of aircraft data to test against"
    )
...
class BulkRuleIdsSerializer(serializers.Serializer):
    ...
    rule_ids = serializers.ListField(child=serializers.IntegerField(), help_text="List of rule IDs")
    enabled = serializers.BooleanField(required=False, help_text="For bulk-toggle: new enabled status")
