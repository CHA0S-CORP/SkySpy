...
def acknowledge_selected(modeladmin, request, queryset):
    ...
    updated = queryset.filter(acknowledged=False).update(acknowledged=True, acknowledged_at=timezone.now())
    modeladmin.message_user(request, f"{updated} item(s) acknowledged.", messages.SUCCESS)
...
def enable_selected(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{updated} item(s) enabled.", messages.SUCCESS)
...
def disable_selected(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{updated} item(s) disabled.", messages.SUCCESS)
...
def activate_selected(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{updated} item(s) activated.", messages.SUCCESS)
...
def deactivate_selected(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{updated} item(s) deactivated.", messages.SUCCESS)
...
def archive_selected(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{updated} item(s) archived.", messages.SUCCESS)
...
def unarchive_selected(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{updated} item(s) unarchived.", messages.SUCCESS)
...
def mark_verified(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{updated} item(s) marked as verified.", messages.SUCCESS)
...
def revoke_api_keys(modeladmin, request, queryset):
    ...
    updated = queryset.update(is_active=False)
    modeladmin.message_user(request, f"{updated} API key(s) revoked.", messages.SUCCESS)
...
def extend_expiration_30_days(modeladmin, request, queryset):
    ...
    for obj in queryset:
        ...
        count += 1
    modeladmin.message_user(request, f"{count} API key(s) expiration extended by 30 days.", messages.SUCCESS)
