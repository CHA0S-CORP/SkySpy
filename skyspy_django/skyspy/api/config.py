...
from skyspy.models.config import SystemConfig, ConfigAuditLog, CATEGORY_CHOICES
...
class ConfigViewSet(viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        responses={200: ConfigDetailSerializer}
    )
    def retrieve(self, request, pk=None):
        ...
        serializer = ConfigDetailSerializer(
            config,
            context={'request': request, 'reveal': reveal}
        )
    ...
    @extend_schema(
        ...
        request=ConfigBulkUpdateSerializer,
        responses={200: dict}
    )
    ...
    @extend_schema(
        summary="Reset configurations to defaults",
        request=ConfigResetSerializer,
        responses={200: dict}
    )
    @action(detail=False, methods=['post'])
    ...
    @extend_schema(
        ...
        request=ConfigImportSerializer,
        responses={200: ConfigImportResultSerializer}
    )
    @action(detail=False, methods=['post'])
    def import_config(self, request):
        ...
        # Actual import
        user = request.user if request.user.is_authenticated else None
        imported, skipped, errors = SystemConfig.import_config(
            configs,
            user=user,
    ...
    @extend_schema(
        ...
        responses={200: ConfigCategorySerializer}
    )
    ...
    def reveal(self, request, pk=None):
        ...
        if not config.is_sensitive:
            return Response(
                {'error': 'Configuration is not sensitive'},
                status=status.HTTP_400_BAD_REQUEST
            )
