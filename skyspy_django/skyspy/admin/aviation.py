...
from skyspy.admin.filters import ArchivedFilter, ObservationTimeDateRangeFilter
from skyspy.admin.mixins import ExportCSVMixin
from skyspy.models import CachedAirport, CachedGeoJSON, CachedNavaid, CachedPirep
...
class CachedAirportAdmin(admin.ModelAdmin):
    ...
    def refresh_airports(self, request, queryset):
        ...
        self.message_user(
            request, "Airport refresh task has been queued. This may take a few moments.", messages.SUCCESS
        )
...
class CachedPirepAdmin(ExportCSVMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("pirep_id", "report_type", "aircraft_type")}),
        ("Location", {"fields": ("location", "latitude", "longitude", "flight_level", "altitude_ft")}),
        ("Time", {"fields": ("observation_time",)}),
        ("Turbulence", {"fields": ("turbulence_type", "turbulence_freq", "turbulence_base_ft", "turbulence_top_ft")}),
        ("Icing", {"fields": ("icing_type", "icing_intensity", "icing_base_ft", "icing_top_ft")}),
        (
            "Weather",
            {"fields": ("sky_cover", "visibility_sm", "weather", "temperature_c", "wind_dir", "wind_speed_kt")},
        ),
        ("Archive", {"fields": ("is_archived", "archived_at", "archive_reason")}),
    )
