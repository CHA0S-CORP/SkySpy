...
class SightingViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(queryset, many=True)
        return Response(
            {"results": serializer.data, "count": len(serializer.data), "limited": total_count > MAX_QUERY_RESULTS}
        )
...
class HistoryViewSet(viewsets.ViewSet):
    ...
    def stats(self, request):
        ...
        # Get sighting stats
        sighting_stats = AircraftSighting.objects.filter(timestamp__gte=cutoff).aggregate(
            total=Count("id"),
        ...
        # Get session stats
        session_stats = AircraftSession.objects.filter(last_seen__gte=cutoff).aggregate(
            total=Count("id"),
    ...
    def trends(self, request):
        ...
        # Calculate summary
        total_unique = AircraftSighting.objects.filter(timestamp__gte=cutoff).values("icao_hex").distinct().count()
    ...
    def top_performers(self, request):
        ...
        # Furthest distance
        furthest = sessions.filter(max_distance_nm__isnull=False).order_by("-max_distance_nm")[:limit]

        # Highest altitude
        highest = sessions.filter(max_altitude__isnull=False).order_by("-max_altitude")[:limit]

        # Most positions
        most_positions = sessions.order_by("-total_positions")[:limit]

        # Closest approach
        closest = sessions.filter(min_distance_nm__isnull=False).order_by("min_distance_nm")[:limit]
    ...
    def analytics_speed(self, request):
        ...
        speed_data = sightings.values("icao_hex", "callsign").annotate(
            max_speed=Max("ground_speed"), avg_speed=Avg("ground_speed"), sample_count=Count("id")
        )
        ...
        # By aircraft type
        type_data = (
            sightings.filter(aircraft_type__isnull=False)
            .values("aircraft_type")
            .annotate(avg_max_speed=Avg("ground_speed"), peak_speed=Max("ground_speed"), count=Count("id"))
    ...
    def analytics_correlation(self, request):
        ...
        distance_bands = (
            sightings.filter(distance_nm__isnull=False, altitude_baro__isnull=False)
            .annotate(
                distance_band=Case(
            ...
            .annotate(avg_altitude=Avg("altitude_baro"), sample_count=Count("id"))
        )
        ...
        if len(scatter_result) > 10:
            ...
            if denom != 0:
                ...
                trend_line = {
                    ...
                    "interpretation": f"RSSI decreases by {abs(round(slope * 10, 2))} dB per 10nm"
                    if slope < 0
    ...
    def analytics_antenna_summary(self, request):
        ...
        coverage_data = (
            sightings.filter(track__isnull=False)
    ...
    def time_comparison_seasonal(self, request):
        ...
        from skyspy.services.time_comparison_stats import calculate_seasonal_trends, get_seasonal_trends
    ...
    def time_comparison_day_night(self, request):
        ...
        from skyspy.services.time_comparison_stats import calculate_day_night_ratio, get_day_night_ratio
