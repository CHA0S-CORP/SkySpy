...
from skyspy.auth.permissions import HasPermission, IsAdminUser, IsSuperAdmin
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def assign_role(self, request, pk=None):
        ...
        if not role_id and not role_name:
            return Response({"error": "Role ID or role name is required"}, status=status.HTTP_400_BAD_REQUEST)

        try:
            role = Role.objects.get(id=role_id) if role_id else Role.objects.get(name=role_name)
...
class RoleViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.is_system:
            return Response({"error": "Cannot delete system roles"}, status=status.HTTP_403_FORBIDDEN)
    ...
    def initialize_defaults(self, request):
        ...
        if not request.user.is_superuser:
            return Response({"error": "Only superusers can initialize default roles"}, status=status.HTTP_403_FORBIDDEN)
        ...
        if created_roles:
            return Response({"message": f"Created {len(created_roles)} default roles", "roles": created_roles})
        else:
...
class UserRoleViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = self.filter_queryset(self.get_queryset())
        serializer = self.get_serializer(queryset, many=True)
        return Response({"assignments": serializer.data, "count": len(serializer.data)})
...
class APIKeyViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = self.filter_queryset(self.get_queryset())
        serializer = self.get_serializer(queryset, many=True)
        return Response({"api_keys": serializer.data, "count": len(serializer.data)})
...
class FeatureAccessViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = self.filter_queryset(self.get_queryset())
        serializer = self.get_serializer(queryset, many=True)
        return Response({"features": serializer.data, "count": len(serializer.data)})
    ...
    def initialize_defaults(self, request):
        ...
        if created_features:
            return Response(
                {"message": f"Created {len(created_features)} feature access entries", "features": created_features}
            )
        else:
...
class OIDCClaimMappingViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = self.filter_queryset(self.get_queryset())
        serializer = self.get_serializer(queryset, many=True)
        return Response({"mappings": serializer.data, "count": len(serializer.data)})
