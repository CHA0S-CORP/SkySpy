...
class AntennaAnalyticsViewSet(ViewSet):
    ...
    def list(self, request):
        ...
        if analytics:
            return Response({"status": "ok", "cached": True, "data": analytics})

        # If no cached data, try to get latest snapshot
        latest = AntennaAnalyticsSnapshot.get_latest()
        if latest:
            return Response({"status": "ok", "cached": False, "data": latest.to_dict()})

        return Response(
            {"status": "no_data", "cached": False, "data": None, "message": "No antenna analytics available yet"}
        )
    ...
    def history(self, request):
        ...
        snapshots = AntennaAnalyticsSnapshot.objects.filter(
            timestamp__gte=cutoff, snapshot_type=snapshot_type
        ).order_by("timestamp")

        data = [snapshot.to_dict() for snapshot in snapshots]

        return Response({"hours": hours, "snapshot_type": snapshot_type, "count": len(data), "snapshots": data})
    ...
    def trends(self, request):
        ...
        snapshots = AntennaAnalyticsSnapshot.objects.filter(timestamp__gte=cutoff, snapshot_type="scheduled").order_by(
            "timestamp"
        )
            ...
            return Response(
                {
                    ...
                    "interval": interval,
                    "trends": [],
                    "summary": {"avg_max_range": None, "peak_max_range": None, "avg_coverage": None, "data_points": 0},
                }
            )
        ...
        return Response({"hours": hours, "interval": interval, "trends": trend_data, "summary": summary})
    ...
    def coverage(self, request):
        ...
        snapshot = (
            AntennaAnalyticsSnapshot.objects.filter(timestamp__gte=cutoff, range_by_direction__isnull=False)
            .exclude(range_by_direction={})
            .order_by("-timestamp")
            .first()
        )

        if not snapshot:
            return Response({"hours": hours, "coverage": None, "message": "No coverage data available"})
        ...
        return Response(
            {
                ...
                "summary": {
                    ...
                    "best_sector": max(
                        (s for s in sectors if s["has_data"]), key=lambda x: x["max_range_nm"] or 0, default=None
                    ),
                    "weakest_sector": min(
    ...
    def cleanup(self, request):
        ...
        deleted_count, _ = AntennaAnalyticsSnapshot.objects.filter(timestamp__lt=cutoff).delete()

        return Response({"deleted_count": deleted_count, "retention_days": days, "cutoff": cutoff.isoformat() + "Z"})
