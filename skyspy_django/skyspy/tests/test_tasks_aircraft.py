...
from skyspy.models import AircraftSession, AircraftSighting, SafetyEvent
...
class PollAircraftTaskTest(TestCase):
    ...
    @override_settings(ULTRAFEEDER_URL="http://test-feeder:8080", FEEDER_LAT=47.5, FEEDER_LON=-122.0)
...
class CleanupSessionsTaskTest(TestCase):
    ...
    def test_cleanup_sessions_identifies_stale(self):
        ...
        AircraftSession.objects.filter(pk=fresh_session.pk).update(last_seen=now - timedelta(minutes=5))
        ...
        AircraftSession.objects.filter(pk=stale_session.pk).update(last_seen=now - timedelta(minutes=40))
...
class UpdateAircraftSessionsTaskTest(TestCase):
    ...
    def test_update_sessions_updates_existing_session(self):
        ...
        AircraftSession.objects.filter(pk=session.pk).update(last_seen=now - timedelta(minutes=1))
...
class UpdateSafetyStatsTaskTest(TestCase):
    ...
    def test_update_safety_stats_with_events(self):
        ...
        # Force old timestamp
        SafetyEvent.objects.filter(pk=old_event.pk).update(timestamp=now - timedelta(hours=25))
