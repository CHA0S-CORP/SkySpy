...
class TestLiveAircraftListing:
    ...
    def test_emergency_squawks_flagged(self, api_client, cached_aircraft_with_emergency):
        ...
        emergency_aircraft = next((ac for ac in data["aircraft"] if ac.get("squawk") == "7700"), None)
...
class TestPermissionChecks:
    ...
    def test_public_mode_sightings_accessible(self, api_client, auth_mode_public):
        """Test that in public mode, sightings are accessible."""
        AircraftSightingFactory.create_batch(5)
        response = api_client.get("/api/v1/sightings/")
        assert response.status_code == status.HTTP_200_OK

    def test_public_mode_sessions_accessible(self, api_client, auth_mode_public):
        """Test that in public mode, sessions are accessible."""
        AircraftSessionFactory.create_batch(3)
        response = api_client.get("/api/v1/sessions/")
        assert response.status_code == status.HTTP_200_OK

    def test_authenticated_mode_requires_auth(self, api_client, auth_mode_authenticated, feature_access_authenticated):
    ...
    def test_viewer_can_access_aircraft(self, viewer_client, cached_aircraft_with_emergency):
    ...
    def test_operator_can_access_aircraft(self, operator_client, cached_aircraft_with_emergency):
        """Test that operator role can access aircraft data."""
        response = operator_client.get("/api/v1/aircraft/")
        assert response.status_code == status.HTTP_200_OK

    def test_analyst_can_access_military_details(self, analyst_client, cached_aircraft_with_emergency):
    ...
    def test_hybrid_mode_respects_feature_access(self, api_client, cached_aircraft_with_emergency, auth_mode_hybrid):
...
class TestAircraftTrackingIntegration:
    ...
    def test_full_aircraft_tracking_workflow(self, api_client, cached_aircraft_with_emergency):
    ...
    def test_consistency_between_endpoints(self, api_client, cached_aircraft_with_emergency):
    ...
    def test_sighting_to_session_relationship(self, api_client):
        ...
        icao = "A12345"
        AircraftSessionFactory(icao_hex=icao)
        AircraftSightingFactory.create_batch(5, icao_hex=icao)
    ...
    def test_all_aircraft_endpoints_return_json(self, api_client, cached_aircraft_with_emergency):
    ...
    def test_emergency_aircraft_in_stats(self, api_client, cached_aircraft_with_emergency):
