...
class TestJWTAuthenticationFlow:
    ...
    def test_login_with_valid_credentials_returns_tokens(self, api_client, create_user, viewer_role):
        ...
        response = api_client.post(
            "/api/v1/auth/login", {"username": "testuser", "password": "SecurePass123!"}, format="json"
        )
    ...
    def test_login_with_invalid_credentials_returns_401(self, api_client, create_user, viewer_role):
        ...
        response = api_client.post(
            "/api/v1/auth/login", {"username": "testuser", "password": "WrongPassword!"}, format="json"
        )
    ...
    def test_login_with_nonexistent_user_returns_401(self, api_client):
        ...
        response = api_client.post(
            "/api/v1/auth/login", {"username": "nonexistent_user", "password": "SomePassword123!"}, format="json"
        )
    ...
    def test_login_with_email_as_username(self, api_client, create_user, viewer_role):
        ...
        user, profile = create_user("testuser", password="SecurePass123!", email="test@example.com", role=viewer_role)

        response = api_client.post(
            "/api/v1/auth/login", {"username": "test@example.com", "password": "SecurePass123!"}, format="json"
        )
...
class TestTokenRefreshFlow:
    ...
    def test_refresh_with_invalid_refresh_token_returns_401(self, api_client):
        ...
        response = api_client.post("/api/v1/auth/refresh", {"refresh": "invalid.refresh.token"}, format="json")
...
class TestLogoutFlow:
    ...
    def test_subsequent_refresh_fails_after_logout(self, api_client, create_user, viewer_role):
        ...
        api_client.credentials(HTTP_AUTHORIZATION=f"Bearer {refresh.access_token}")
        api_client.post("/api/v1/auth/logout", {"refresh": str(refresh)}, format="json")
...
class TestAPIKeyAuthentication:
    ...
    def test_api_key_scopes_are_respected(self, api_client, operator_user, create_api_key, db):
        ...
        api_key, raw_key = create_api_key(operator_user, name="Limited Key", scopes=["aircraft.view"])
...
class TestPermissionChecks:
    ...
    def test_operator_can_edit_own_alerts(self, operator_client, operator_user, db):
        ...
        response = operator_client.patch(f"/api/v1/alerts/rules/{alert.id}/", {"name": "Updated Alert"}, format="json")
    ...
    def test_superadmin_has_full_access(self, superadmin_client, operator_user, db):
        ...
        response = superadmin_client.patch(
            f"/api/v1/alerts/rules/{other_alert.id}/", {"name": "Superadmin Modified"}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
...
class TestAuthModeBehavior:
    ...
    def test_authenticated_mode_allows_authenticated_users(self, viewer_client, auth_mode_authenticated):
...
class TestProfileEndpoint:
    ...
    def test_patch_profile_updates_display_name(self, viewer_client):
        ...
        response = viewer_client.patch("/api/v1/auth/profile", {"display_name": "New Display Name"}, format="json")
    ...
    def test_patch_profile_updates_preferences(self, viewer_client):
        ...
        response = viewer_client.patch("/api/v1/auth/profile", {"preferences": preferences}, format="json")
...
class TestFeatureAccessConfiguration:
    ...
    def test_feature_public_access_allows_anonymous(self, api_client, feature_access_public):
...
class TestAuthEdgeCasesAndSecurity:
    ...
    def test_login_creates_profile_if_missing(self, api_client, db):
        ...
        user = User.objects.create_user(username="newuser", password="SecurePass123!", email="new@example.com")

        response = api_client.post(
            "/api/v1/auth/login", {"username": "newuser", "password": "SecurePass123!"}, format="json"
        )
    ...
    def test_concurrent_logins_allowed(self, api_client, create_user, viewer_role):
        ...
        response1 = api_client.post(
            "/api/v1/auth/login", {"username": "multilogin", "password": "SecurePass123!"}, format="json"
        )
    ...
    def test_local_auth_can_be_disabled(self, api_client, create_user, viewer_role):
        ...
        with override_settings(LOCAL_AUTH_ENABLED=False):
            response = api_client.post(
                "/api/v1/auth/login", {"username": "localuser", "password": "SecurePass123!"}, format="json"
            )
...
class TestPasswordChange:
    ...
    def test_password_change_with_correct_current_password(self, viewer_client, viewer_user):
        ...
        response = client.post(
            ...
            {"current_password": "OldPassword123!", "new_password": "NewSecurePass123!"},
            format="json",
        )
    ...
    def test_password_change_with_wrong_current_password_fails(self, viewer_client, viewer_user):
        """Password change fails with wrong current password."""
        response = viewer_client.post(
            "/api/v1/auth/password",
            {"current_password": "WrongPassword123!", "new_password": "NewSecurePass123!"},
            format="json",
        )
...
class TestRoleExpiration:
    ...
    def test_active_role_included_in_permissions(self, api_client, create_user, operator_role):
        ...
        UserRole.objects.create(user=user, role=operator_role, expires_at=timezone.now() + timedelta(days=30))
    ...
    def test_permanent_role_always_active(self, api_client, create_user, viewer_role):
        ...
        UserRole.objects.create(user=user, role=viewer_role, expires_at=None)
