...
class ArchiveViewSet(viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(name="icao", type=str, description="Filter by airport ICAO code"),
            OpenApiParameter(name="type", type=str, description="NOTAM type filter (D, FDC, TFR, GPS)"),
            OpenApiParameter(name="search", type=str, description="Text search in NOTAM content"),
            OpenApiParameter(name="limit", type=int, description="Maximum number of results (default: 50)"),
            OpenApiParameter(name="offset", type=int, description="Offset for pagination (default: 0)"),
        ],
    )
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(name="days", type=int, description="Number of days to look back (default: 30)"),
            OpenApiParameter(name="icao", type=str, description="Filter by airport/location ICAO code"),
            OpenApiParameter(name="report_type", type=str, description="Report type (UA=routine, UUA=urgent)"),
            OpenApiParameter(name="turbulence", type=str, description="Filter by turbulence type"),
            OpenApiParameter(name="icing", type=str, description="Filter by icing type"),
            OpenApiParameter(name="search", type=str, description="Text search in PIREP content"),
            OpenApiParameter(name="limit", type=int, description="Maximum number of results (default: 50)"),
            OpenApiParameter(name="offset", type=int, description="Offset for pagination (default: 0)"),
        ],
    )
    ...
    def pireps(self, request):
        ...
        if search:
            queryset = queryset.filter(
                Q(raw_text__icontains=search) | Q(location__icontains=search) | Q(aircraft_type__icontains=search)
            )
    ...
    def stats(self, request):
        ...
        pirep_turbulence = dict(
            CachedPirep.objects.filter(is_archived=True, turbulence_type__isnull=False)
        ...
        pirep_icing = dict(
            CachedPirep.objects.filter(is_archived=True, icing_type__isnull=False)
            .values("icing_type")
            .annotate(count=Count("id"))
    ...
    def notam_detail(self, request, notam_id=None):
        ...
        try:
            ...
            return Response({"error": f"NOTAM {notam_id} not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def pirep_detail(self, request, pirep_id=None):
        ...
        try:
            ...
            return Response({"error": f"PIREP {pirep_id} not found"}, status=status.HTTP_404_NOT_FOUND)
