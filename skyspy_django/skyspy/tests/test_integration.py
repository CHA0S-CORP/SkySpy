...
from skyspy.tasks.transcription import process_transcription_queue, transcribe_audio
...
class TestAcarsFlow:
    ...
    async def test_store_acars_message(self, db, acars_service, mock_acars_message):
        ...
        message = await sync_to_async(AcarsMessage.objects.filter(icao_hex="A12345").first, thread_sensitive=True)()
...
class TestTranscriptionFlow:
    ...
    def test_transcription_success(self, mock_client_class, mock_read_local, db, temp_audio_dir, sample_audio_file):
        ...
        mock_response.json.return_value = {
            ...
            "confidence": 0.95,
            "language": "en",
            "segments": [{"start": 0.0, "end": 3.0, "text": "United four five six, cleared for takeoff."}],
        }
        mock_response.raise_for_status = MagicMock()
    ...
    def test_transcription_file_not_found(self, db, temp_audio_dir):
        ...
        assert (
            "fetch" in transmission.transcription_error.lower()
            or "not found" in transmission.transcription_error.lower()
        )
...
class TestPerformance:
    ...
    def test_alert_evaluation_performance(self, db, alert_service):
        ...
        start = time.time()
        alert_service.check_alerts(aircraft_data)
        elapsed = time.time() - start
    ...
    def test_safety_monitor_performance(self, db, safety_monitor):
        ...
        start = time.time()
        safety_monitor.update_aircraft(aircraft_data)
        elapsed = time.time() - start
