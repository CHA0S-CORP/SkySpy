...
class SkyspyUser(models.Model):
    ...
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name="skyspy_profile")
    ...
    oidc_issuer = models.CharField(max_length=500, blank=True, null=True, help_text="OIDC issuer URL")
    oidc_claims = models.JSONField(blank=True, null=True, help_text="Cached OIDC claims from last login")
    ...
    preferences = models.JSONField(
        default=dict, blank=True, help_text="User preferences (map settings, notifications, etc.)"
    )
...
class UserRole(models.Model):
    ...
    expires_at = models.DateTimeField(blank=True, null=True, help_text="Role assignment expires at this time")
...
class APIKey(models.Model):
    ...
    scopes = models.JSONField(
        default=list, blank=True, help_text="Permission scopes for this key (empty = all user permissions)"
    )
...
class FeatureAccess(models.Model):
    ...
    feature = models.CharField(max_length=30, choices=FEATURE_CHOICES, unique=True, primary_key=True)
    ...
    settings = models.JSONField(default=dict, blank=True, help_text="Feature-specific access settings")
...
class OIDCClaimMapping(models.Model):
    ...
    name = models.CharField(max_length=100, help_text="Descriptive name for this mapping")

    # The OIDC claim to match (e.g., 'groups', 'roles', 'email')
    claim_name = models.CharField(max_length=100, help_text="OIDC claim name to match (e.g., groups, roles)")

    # How to match the claim value
    match_type = models.CharField(max_length=20, choices=MATCH_TYPE_CHOICES, default="exact")

    # The value to match
    claim_value = models.CharField(max_length=255, help_text="Value to match in the claim")

    # Role to assign when matched
    role = models.ForeignKey(Role, on_delete=models.CASCADE, related_name="claim_mappings")
        ...
        for value in claim_values:
            if self.match_type == "exact":
                ...
                if len(self.claim_value) > MAX_REGEX_PATTERN_LENGTH:
                    logger.warning(
                        f"OIDC claim mapping regex pattern too long ({len(self.claim_value)} chars), skipping"
                    )
                    return False
                try:
...
ALL_PERMISSIONS = [perm for perms in FEATURE_PERMISSIONS.values() for perm in perms]
