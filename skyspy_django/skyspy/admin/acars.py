...
class AcarsMessageAdmin(ExportCSVMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ("Timestamp & Source", {"fields": ("timestamp", "source", "channel", "frequency", "station_id")}),
        ("Aircraft", {"fields": ("icao_hex", "registration", "callsign")}),
        ("Message", {"fields": ("label", "block_id", "msg_num", "ack", "mode", "text")}),
        ("Decoded Content", {"fields": ("decoded",), "classes": ("collapse",)}),
        ("Signal", {"fields": ("signal_level", "error_count"), "classes": ("collapse",)}),
    )
    ...
    def decoded_display(self, obj):
        ...
        if obj.decoded:
            try:
                ...
                return format_html(
                    '<pre style="margin: 0; font-size: 11px; white-space: pre-wrap;">{}</pre>', escape(formatted)
                )
