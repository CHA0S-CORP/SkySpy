...
class MessageStore:
    ...
    def search(self, flight: str | None = None, tail: str | None = None) -> list:
        results = []
        for msg in self.messages:
            if (
                flight
                and flight.upper() in msg.get("flight", "").upper()
                or tail
                and tail.upper() in msg.get("tail", "").upper()
            ):
                results.append(msg)
        return results
...
@app.get("/api/search")
async def search_messages(flight: str | None = None, tail: str | None = None):
...
if __name__ == "__main__":
    uvicorn.run("processor:app", host="0.0.0.0", port=API_PORT, log_level=LOG_LEVEL.lower())
