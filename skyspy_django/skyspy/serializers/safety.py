...
class SafetyStatsSerializer(serializers.Serializer):
    ...
    monitoring_enabled = serializers.BooleanField(help_text="Whether safety monitoring is active")
    thresholds = serializers.DictField(help_text="Current safety thresholds")
    time_range_hours = serializers.IntegerField(default=24, help_text="Statistics time range")
    ...
    events_by_type_severity = serializers.DictField(help_text="Per-type severity breakdown")
    total_events = serializers.IntegerField(help_text="Total events in time range")
    unique_aircraft = serializers.IntegerField(help_text="Unique aircraft involved in events")
    event_rate_per_hour = serializers.FloatField(help_text="Average events per hour")
    events_by_hour = serializers.ListField(child=serializers.DictField(), help_text="Hourly event distribution")
    top_aircraft = serializers.ListField(child=serializers.DictField(), help_text="Aircraft with most events")
    recent_events = serializers.ListField(child=serializers.DictField(), help_text="Most recent events")
...
class AircraftSafetyStatsSerializer(serializers.Serializer):
    ...
    callsign = serializers.CharField(required=False, allow_null=True, help_text="Last known callsign")
    total_events = serializers.IntegerField(help_text="Total events involving this aircraft")
    ...
    worst_severity = serializers.CharField(required=False, allow_null=True, help_text="Worst severity level")
    last_event_time = serializers.CharField(required=False, allow_null=True, help_text="Most recent event timestamp")
    last_event_type = serializers.CharField(required=False, allow_null=True, help_text="Most recent event type")


class AircraftSafetyStatsListSerializer(serializers.Serializer):
    """Response for aircraft safety statistics."""

    aircraft = AircraftSafetyStatsSerializer(many=True, help_text="Aircraft statistics")
