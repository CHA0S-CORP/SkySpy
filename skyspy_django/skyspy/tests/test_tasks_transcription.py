...
class ExtractCallsignsTaskTest(TestCase):
    ...
    def test_extract_airline_callsigns(self):
        ...
        self.assertTrue(
            any("UAL" in cs for cs in airline_callsigns) or any("United" in name for name in airline_names if name)
        )
        self.assertTrue(
            any("DAL" in cs for cs in airline_callsigns) or any("Delta" in name for name in airline_names if name)
        )
    ...
    def test_extract_multiple_callsign_types(self):
        ...
        types = {c.get("type") for c in callsigns}
