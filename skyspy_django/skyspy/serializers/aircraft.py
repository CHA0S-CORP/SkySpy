...
class AircraftSerializer(serializers.Serializer):
    ...
    hex = serializers.CharField(source="icao_hex", help_text="ICAO 24-bit hex identifier")
    flight = serializers.CharField(
        source="callsign", required=False, allow_null=True, help_text="Callsign/flight number"
    )
    type = serializers.CharField(
        source="aircraft_type", required=False, allow_null=True, help_text="Aircraft type code (ICAO)"
    )
    alt = serializers.IntegerField(
        source="altitude_baro", required=False, allow_null=True, help_text="Barometric altitude in feet"
    )
    gs = serializers.FloatField(
        source="ground_speed", required=False, allow_null=True, help_text="Ground speed in knots"
    )
    vr = serializers.IntegerField(
        source="vertical_rate", required=False, allow_null=True, help_text="Vertical rate in feet/minute"
    )
    distance_nm = serializers.FloatField(
        required=False, allow_null=True, help_text="Distance from feeder in nautical miles"
    )
    squawk = serializers.CharField(required=False, allow_null=True, help_text="Transponder squawk code")
    category = serializers.CharField(required=False, allow_null=True, help_text="Aircraft category (A0-D7)")
    rssi = serializers.FloatField(required=False, allow_null=True, help_text="Signal strength in dBFS")
    lat = serializers.FloatField(
        source="latitude", required=False, allow_null=True, help_text="Latitude in decimal degrees"
    )
    lon = serializers.FloatField(
        source="longitude", required=False, allow_null=True, help_text="Longitude in decimal degrees"
    )
    track = serializers.FloatField(required=False, allow_null=True, help_text="Ground track in degrees")
    military = serializers.BooleanField(source="is_military", default=False, help_text="Military aircraft flag")
    emergency = serializers.BooleanField(source="is_emergency", default=False, help_text="Emergency squawk detected")


class AircraftListSerializer(serializers.Serializer):
    """Response containing list of aircraft with metadata."""

    aircraft = serializers.ListField(child=serializers.DictField(), help_text="List of aircraft currently tracked")
    count = serializers.IntegerField(help_text="Number of aircraft in response")
    now = serializers.FloatField(required=False, allow_null=True, help_text="Unix timestamp from data source")
    messages = serializers.IntegerField(default=0, help_text="Total messages received by feeder")
...
class TopAircraftSerializer(serializers.Serializer):
    ...
    closest = serializers.ListField(child=serializers.DictField(), help_text="Aircraft closest to feeder")
    highest = serializers.ListField(child=serializers.DictField(), help_text="Aircraft at highest altitude")
    fastest = serializers.ListField(child=serializers.DictField(), help_text="Aircraft with highest ground speed")
    climbing = serializers.ListField(child=serializers.DictField(), help_text="Aircraft with highest climb rate")
    military = serializers.ListField(child=serializers.DictField(), help_text="Military aircraft detected")
...
class AircraftStatsSerializer(serializers.Serializer):
    ...
    emergency = serializers.ListField(child=serializers.DictField(), help_text="Aircraft squawking emergency")
...
class MatchedRadioCallSerializer(serializers.Serializer):
    ...
    transcript = serializers.CharField(required=False, allow_null=True, help_text="Transcribed text")
    frequency_mhz = serializers.FloatField(required=False, allow_null=True, help_text="Radio frequency")
    channel_name = serializers.CharField(required=False, allow_null=True, help_text="Channel name")
    duration_seconds = serializers.FloatField(required=False, allow_null=True, help_text="Audio duration")
    confidence = serializers.FloatField(required=False, allow_null=True, help_text="Callsign extraction confidence")
    raw_text = serializers.CharField(required=False, allow_null=True, help_text="Raw callsign text from transcript")
    audio_url = serializers.CharField(required=False, allow_null=True, help_text="URL to audio file")
...
class AircraftInfoSerializer(serializers.ModelSerializer):
    ...
    matched_radio_calls = MatchedRadioCallSerializer(many=True, required=False, allow_null=True)
...
class AircraftPhotoSerializer(serializers.Serializer):
    ...
    photo_url = serializers.CharField(required=False, allow_null=True, help_text="Full-size photo URL")
    thumbnail_url = serializers.CharField(required=False, allow_null=True, help_text="Thumbnail URL")
    photographer = serializers.CharField(required=False, allow_null=True, help_text="Photographer credit")
    source = serializers.CharField(required=False, allow_null=True, help_text="Photo source")


class BulkAircraftInfoSerializer(serializers.Serializer):
    """Response for bulk aircraft info lookup."""

    aircraft = serializers.DictField(help_text="Map of ICAO hex to aircraft info")
    found = serializers.IntegerField(help_text="Number of aircraft found in cache")
    requested = serializers.IntegerField(help_text="Number of valid ICAO codes requested")
...
class AircraftInfoCacheStatsSerializer(serializers.Serializer):
    ...
    retry_after_hours = serializers.IntegerField(help_text="Retry failed lookups after hours")
