...
from skyspy.admin.filters import DateRangeFilter, TimestampDateRangeFilter
...
class AircraftSightingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identification", {"fields": ("timestamp", "icao_hex", "callsign", "squawk", "source")}),
        ("Position", {"fields": ("latitude", "longitude", "altitude_baro", "altitude_geom", "distance_nm")}),
        ("Flight Data", {"fields": ("ground_speed", "track", "vertical_rate")}),
        ("Signal", {"fields": ("rssi",)}),
        ("Classification", {"fields": ("category", "aircraft_type", "is_military", "is_emergency")}),
    )
...
class AircraftSessionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Altitude Range", {"fields": ("min_altitude", "max_altitude")}),
        ("Distance Range", {"fields": ("min_distance_nm", "max_distance_nm")}),
        ("Signal Quality", {"fields": ("min_rssi", "max_rssi", "max_vertical_rate")}),
    )
...
class AircraftInfoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("History", {"fields": ("year_built", "first_flight_date", "delivery_date", "airframe_hours")}),
        (
            "Operator",
            {
                "fields": (
        ...
        ("Flags", {"fields": ("category", "is_military", "is_interesting", "is_pia", "is_ladd")}),
        (
            "Photos",
            {
                "fields": (
        ...
        ("Cache", {"fields": ("extra_data", "created_at", "updated_at", "fetch_failed"), "classes": ("collapse",)}),
    )
    ...
    def photo_thumbnail_display(self, obj):
        ...
        if obj.photo_thumbnail_url:
            ...
            return format_html(
                '<img src="{}" style="max-width: 60px; max-height: 40px; object-fit: cover;" />', obj.photo_url
            )
        return "-"
    ...
    def refresh_aircraft_info(self, request, queryset):
        ...
        if refreshed > 0:
            self.message_user(request, f"Successfully refreshed info for {refreshed} aircraft.", messages.SUCCESS)
        if failed > 0:
            self.message_user(request, f"Failed to refresh info for {failed} aircraft.", messages.WARNING)
