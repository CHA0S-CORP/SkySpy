 ...
 */
export function useReplayState({ apiBase, wsRequest, wsConnected }) {
            ...
            try {
              ...
              if (wsRequest && wsConnected) {
                ...
                const res = await fetch(
                  `${apiBase}/api/v1/sightings?icao_hex=${icao}&hours=2&limit=500`,
                  {
  ...
  const initializeMap = useCallback(
    (eventKey, event, containerRef) => {
      ...
      eventMarker.bindPopup(
        `<b>Event Location</b><br>${event.event_type?.replace(/_/g, ' ')}<br>${new Date(event.timestamp).toLocaleString()}`
      );
